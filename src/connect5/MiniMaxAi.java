...
public class MiniMaxAi {
		...
		public Move max(Board board, int depth) {
	         ...
	         */
			if((board.check_Game_Over()) || (depth == maxDepth)) {
				Move lastMove = new Move(board.getLastMove().getRow(), board.getLastMove().getCol(), board.evaluation());
				return lastMove;
			}
	        //The children-moves of the state are calculated
			ArrayList<Board> children = new ArrayList<Board>(board.get_Children(Constants.Player1));
		...
		public Move min(Board board, int depth) {
			...
			ArrayList<Board> children = new ArrayList<Board>(board.get_Children(Constants.Player2));
